	...
	private Json next() {
		if(c == '\"') {
			...
			do {
				...
				if(c == '\\') {
					...
					case 'u':
						...
						continue;
					default:
						throw new JsonParserException("Invalid escape character: \\"+c+" at position "+n);
					}
				} else if(c == '\"') {
					c = s[n++];
			...
			do {
				...
				if(c != ',') throw new JsonParserException("expected ',' or ']' at character "+n);
				c = s[n++];
				while(c == ' ' || c == '\t' || c == '\n' || c == '\r') { c = s[n++]; }
			} while(true);
		} else if(c == '{') {
			...
			do {
				Json k = next();
				if(!k.isString())
					throw new JsonParserException("find non-string key at character "+n);
				String key = k.str();
				while(c == ' ' || c == '\t' || c == '\n' || c == '\r') { c = s[n++]; }
				if(c != ':') throw new JsonParserException("expected ':' at character "+n);
				c = s[n++];
				while(c == ' ' || c == '\t' || c == '\n' || c == '\r') { c = s[n++]; }
				Json value = next();
			...
			if(s[n++] != 'r' || s[n++] != 'u' || s[n++] != 'e')
				throw new JsonParserException("expected 'true' at character "+n);
			if(n != s.length) c = s[n++];
			return new Json(Boolean.TRUE);
		} else if(c == 'f') {
			if(s[n++] != 'a' || s[n++] != 'l' || s[n++] != 's' || s[n++] != 'e')
				throw new JsonParserException("expected 'false' at character "+n);
			if(n != s.length) c = s[n++];
			...
			throw new JsonParserException("expected: '{', '[', '\"', true, false, null, or a number at "+n);
		}
	}
