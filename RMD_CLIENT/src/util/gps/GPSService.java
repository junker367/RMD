...
public class GPSService extends Service implements LocationListener {
	...
	public void onCreate() {
		...
		prefs.registerOnSharedPreferenceChangeListener(new OnSharedPreferenceChangeListener() {
			public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
				Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Preferences Changed!");
			}
		});

		locationManager = (LocationManager) getSystemService(LOCATION_SERVICE);

		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "onCreate'd");
	}
	...
	public void onStart(Intent intent, int startId) {
		...
		try {
			...
			Log.e(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Error al iniciar el servicio en onStart()");
		}
	...
	public void onDestroy() {
		...
		handler = null;
		stopDeteccion();
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Deteniendo grabacion de posiciones...");
	}

	public void onLocationChanged(Location location) {
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "onLocationChanged with Location: " + location);

		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Recibida posicion. Procesando...");

		if ((getPuntoActual() != null) && (location != null) && (isCorrectPosicion(location, getPuntoActual()))) {
			setPuntoActual(location);
		} else {
			Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Es primera posicion recibida. Guardando...");
			setPuntoActual(location);
		}
	}

	private boolean isCorrectPosicion(Location location, Location currentBestLocation) {
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Procesando precision de la posicion recibida...");
		if (currentBestLocation == null) {
			Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "currentBestLocation == null. Posicion correcta. Esto no debería ocurrir!!!");
			return true;
		}
	...
	private boolean intervalTimeLocationIsCorrect(Location location, Location currentBestLocation) {
		...
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Ultima posicion recibida en: " + currentBestLocation.getTime());
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Nueva posicion recibida en: " + location.getTime());
		Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Diferencia de tiempo entre posiciones: " + timeDelta);

		if (entraEnIntervalo) {
			Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Diferencia de tiempo(" + timeDelta + ") mayor/igual que intervalo definido(" + intervaloConfiguracion + "). Posicion correcta.");
			return true;
		} else if (noEntraEnIntervalo) {
			Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Diferencia de tiempo(" + timeDelta + ") menor que intervalo definido(" + intervaloConfiguracion + "). Posicion correcta.");
			return false;
		}
		return false;
	}
	...
	private void inicioDeteccion() {
		if (configuracionGps.intervaloRecepcionSignal != null) {
			...
			Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Iniciando grabacion de posiciones desde " + provider);
		}
	}
		...
		public void run() {
			if (getPuntoActual() != null) {
				if (posicionesSonDistintas()) {
					...
					Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Punto recibido en el gps. Enviada notificacion.");
					...
					Log.d(ConstantesSMS.tagsLogger.TAG_GPS_SERVICE, "Enviada notificacion OK.");
				}
			} else {
				String provider=getServiceProvider();
				if(provider!=null){
